...
# Density plots of the marginal distributions
sm.density(keith$gpa, xlab = "GPA (on a 100-pt. scale)")
sm.density(keith$homework, xlab = "Time spent on homework per week (in hours)")
...
```{r fig.width=6, fig.height = 4, out.width='4in', fig.cap='Coefficient plot for the model regressing GPA on time spent on homework. Uncertainty based on the 95\\% confidence intervals are displayed.', echo=FALSE}
...
The coefficient plot shows the estimates of the regression coefficients (dots) and the uncertainty in those estimates via the confidence intervals (red lines).
...
There are several variations of this plot. For example, below I show a coefficient plot that shows the uncertainty based on the the 68\%, 95\%, and 99\% intervals. Since the coefficient of interest in the research question is the slope, I also omit the intercept from this plot.

```{r fig.width=6, fig.height = 4, out.width='4in', fig.cap='Coefficient plot for the model regressing income on education. Uncertainty based on the 68\\% and 95\\% confidence intervals are displayed.', fig.pos='H', echo=FALSE}
# Add the lower- and upper-limits for the CIs
my_reg = tidy(lm.1) %>%
  mutate(
    LL_99 = estimate - 3 * std.error,
    UL_99 = estimate + 3 * std.error,
...
ggplot(data = my_reg, aes(x = estimate, y = term)) +
  geom_segment(
    aes(x = LL_99, xend = UL_99, y = term, yend = term),
    color = "#2C7FB8",
    alpha = 0.3,
    size = 1.1
    ) +
  ...
  geom_point(shape = 15, color = "#081D58") +
  theme_bw() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()
...
- Mutate new columns for the lower- and upper-limits for the 99\% CIs onto the `tidy()` output.
- Filter to keep only the `homework` term.
- Plot line segments that correspond to the 68\% CIs. Make this color mostly transparent.
- Plot line segments that correspond to the 95\% CIs. Make this color somewhat transparent.
- Plot line segments that correspond to the 99\% CIs. Make this color opaque.
- Plot the estimates for the coefficients as a point.
- Spruce up the plot.


<!-- # Back to the Riverview Example -->
