...
We will also fit a model by regressing income on education level and storing those results in an object called `lm.1`.
...
$$
\hat{\mathrm{GPA}} = 74.29 + 1.21(\mathrm{Time~spent~on~homework}),
$$
...
```{r fig.width=6, fig.height = 3, out.width='4in', fig.cap='Coefficient plot for the model regressing income on education. Uncertainty based on the 95\\% confidence interval is displayed. Note that the intercept has been omitted.', fig.pos='H', echo=TRUE}
# Omit intercept
my_coefs = tidy(lm.1) %>%
